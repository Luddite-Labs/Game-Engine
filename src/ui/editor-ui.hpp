...
void drawToolBar(SDL_Window *window) {
	// ImGui::Begin("Online", NULL, ImGuiWindowFlags_MenuBar);
...
void drawComponent(const RE::Material::Handle &component) {
...
void drawComponent(RE::Mesh::Handle &component) {
	...
	for (auto &primitive : RE::Mesh::getPrimitives(component)) {
		if (ImGui::TreeNode(("Primitive" + std::to_string(primitive_index)).c_str())) {
			ImGui::Text("Vert Count: %d", primitive.vert_count);
			ImGui::Text("Index Count: %d", primitive.index_count);
			switch (primitive.type) {
				case RE::Mesh::Primitive::Type::TRIANGLELIST:
					ImGui::Text("Primitive Type: Triangle list");
					break;
			}
			if (ImGui::TreeNode(("Material##" + std::to_string(primitive_index)).c_str())) {
				drawComponent(primitive.material); //! const correctness
				ImGui::TreePop();
			}
			ImGui::TreePop();
		}
